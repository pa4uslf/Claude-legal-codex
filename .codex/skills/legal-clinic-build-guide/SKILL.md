...
# Codex skill: legal-clinic-build-guide

1. Load `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → role (must be Supervising attorney), practice areas, jurisdiction.
...
5. Write `~/.codex/claude-for-legal/legal-clinic/guides/<practice-area>.md`. Create the `guides/` directory if needed.
...
This is a supervisor skill. Read `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → `## Who's using this` → Role. If the role is not "Supervising attorney," say:
...
If a guide already exists at `~/.codex/claude-for-legal/legal-clinic/guides/<practice-area>.md`, offer: "A guide for [area] already exists at [path]. Do you want to (a) revise it section-by-section, (b) start fresh and overwrite, or (c) see what's there first?"
...
Write to `~/.codex/claude-for-legal/legal-clinic/guides/<practice-area>.md`. Create the `guides/` directory if it doesn't exist. Use this structure:
...
The skill's "output" is the file written at `~/.codex/claude-for-legal/legal-clinic/guides/<practice-area>.md`. The conversation with the supervisor is the interview; the written guide is the artifact.
