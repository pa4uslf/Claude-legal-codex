...
description: >
  Codex CLI version of ip-legal:portfolio. Track the IP portfolio — registrations, renewals, maintenance fees, and use declarations. Use when checking what's renewing, adding or updating an asset, recording a maintenance filing, or auditing the register for gaps, lapses, and use-in-commerce questions. Receives handoffs from prosecution and clearance work.
argument-hint: "[--report [--days N] | --add | --update | --audit]"
---

# Codex Adaptation Notes

This skill was converted from `ip-legal:portfolio` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ip-legal-portfolio`, or by describing the legal task in natural language.
...
# Codex skill: ip-legal-portfolio
...
1. **Follow the workflow below** and read
   `~/.codex/claude-for-legal/ip-legal/portfolio.yaml`.
...
Lives at `~/.codex/claude-for-legal/ip-legal/portfolio.yaml`.
Structure:
...
Output (prepend work-product header per `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` → Outputs):
...
`## Who's using this` in `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
