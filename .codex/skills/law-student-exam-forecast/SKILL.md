...
description: >
  Codex CLI version of law-student:exam-forecast. Analyze past exams from the same professor to surface patterns — subject weighting, recurring issue-spot traps, favored hypo types, policy-vs-doctrine mix — and forecast likely emphases for the upcoming exam. Use when the user says "what's on the exam", "analyze past exams", "predict the exam", or shares past exams.
argument-hint: "[class name, with past exams shared or paths to them]"
---

# Codex Adaptation Notes

This skill was converted from `law-student:exam-forecast` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `law-student-exam-forecast`, or by describing the legal task in natural language.
...
# Codex skill: law-student-exam-forecast

1. Load `~/.codex/claude-for-legal/law-student/CLAUDE.md` → class, professor, exam format, syllabus.
...
7. Write `~/.codex/claude-for-legal/law-student/exam-forecasts/[class]/forecast-[YYYY-MM-DD].md`. Framed as weighting heuristic, not prediction.
...
- `~/.codex/claude-for-legal/law-student/CLAUDE.md` → current classes, exam formats, syllabus if captured
...
Write to `~/.codex/claude-for-legal/law-student/exam-forecasts/[class]/forecast-[YYYY-MM-DD].md`. Versioned — if the student gets another past exam mid-semester, re-run and append.
