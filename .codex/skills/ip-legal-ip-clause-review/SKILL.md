...
description: >
  Codex CLI version of ip-legal:ip-clause-review. Review the IP clauses in an agreement — assignment, ownership, license grants, warranties, indemnities. Use when reviewing IP terms in employment, consulting, SOW, vendor, or licensing agreements, when asked to check the assignment language or license scope, or when an agreement with IP provisions is pasted or attached.
argument-hint: "[file path | Drive link | paste text]"
---

# Codex Adaptation Notes

This skill was converted from `ip-legal:ip-clause-review` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ip-legal-ip-clause-review`, or by describing the legal task in natural language.
...
# Codex skill: ip-legal-ip-clause-review

Reviews the IP clauses in an agreement against the practice profile in `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`.** If placeholders present, stop and prompt: "Run `ip-legal-cold-start-interview` first — I need to learn your practice profile before I can review IP clauses against it."
...
**Before reading the agreement, read `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`.** If it is missing or still contains placeholders, stop and run `ip-legal-cold-start-interview`. The practice profile tells you:
...
**Escalation:** Per `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` → `## Outputs` (it differs by user role — see `## Who's using this`).
