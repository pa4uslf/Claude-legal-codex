...
description: >
  Codex CLI version of ip-legal:cold-start-interview. Run the cold-start interview to learn your IP practice and write your practice profile. Use on first install when the practice profile is missing or still contains placeholders, when re-onboarding with --redo, or when re-probing integrations with --check-integrations after connecting or disconnecting an MCP. This is the ONLY skill that should run on a fresh install.
argument-hint: "[--redo to re-run on an already-configured plugin] [--check-integrations to re-probe integrations only]"
---

# Codex Adaptation Notes

This skill was converted from `ip-legal:cold-start-interview` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ip-legal-cold-start-interview`, or by describing the legal task in natural language.

Configuration for this fork uses `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` instead of the Claude plugin config directory. Shared organization facts live in `~/.codex/claude-for-legal/company-profile.md`.
...
# Codex skill: ip-legal-cold-start-interview

Runs the cold-start interview. First run writes `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` (I'll show you a diff first)."
...
5. **Migration:** If a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.codex/claude-for-legal/cache/ip-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.

6. **Write `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.

7. **Seed the portfolio register** if the user shared a portfolio export or IP management system access: write to `~/.codex/claude-for-legal/ip-legal/portfolio.yaml`. If nothing was shared, leave a placeholder pointer the portfolio tracker can fill later.

8. **Show summary + propose next steps:**
   - "Here's what I heard — `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` is written. What did I get wrong?"
...
Read `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`:
...
The template structure lives at `ip-legal/CLAUDE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed.

If a CLAUDE.md exists at the old cache path `~/.codex/claude-for-legal/cache/ip-legal/*/CLAUDE.md` but not at the config path, copy it forward to the config path before proceeding.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Anaqua isn't connected. In Codex CLI: Settings → Connectors → Add → Anaqua → sign in. In Codex CLI: add the Anaqua MCP to your config or via `/mcp`. This plugin works without it — portfolio lives in `portfolio.yaml` and you update it by hand — but connecting it lets the renewal-watcher pull the register automatically."
...
Write the plugin config following the structure in `ip-legal/CLAUDE.md` (the template). Use their words where you can. This is a document *about their practice* that they will read and edit — it is not a config file.
...
Write to `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` (create parent directories as needed). If the user shared a portfolio export, also seed `~/.codex/claude-for-legal/ip-legal/portfolio.yaml` with the extracted registrations.
...
4. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
...
5. **Before your first clearance**: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex CLI: authorize when a skill prompts you.
