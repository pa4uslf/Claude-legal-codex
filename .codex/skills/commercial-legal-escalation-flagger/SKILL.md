...
description: >
  Codex CLI version of commercial-legal:escalation-flagger. Route a contract issue to the right approver per the escalation matrix in `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`, and draft the ask. Use when the user says "who needs to approve this", "escalate this", "does this need GC sign-off", "route this for approval", or when another skill finds an issue that exceeds the reviewer's authority.
argument-hint: "[describe the issue, or reference a review memo]"
---

# Codex Adaptation Notes

This skill was converted from `commercial-legal:escalation-flagger` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `commercial-legal-escalation-flagger`, or by describing the legal task in natural language.
...
# Codex skill: commercial-legal-escalation-flagger

Names the approver for a contract issue per the `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` escalation matrix and drafts the message so you're not writing "hey got a sec" at 5pm.

## Instructions

1. **Load `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`** → Escalation section. If missing, say so — the practice profile needs editing.
...
Every contracts team has an escalation matrix, written or not. This skill reads the written one (in `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`), matches a contract issue against it, names the approver, and drafts the ask so the lawyer isn't writing "hey do you have a sec" messages at 5pm.
...
Read `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` → `## Escalation`. If it's missing or vague, say so — the cold-start interview should have captured this, and if it didn't, the practice profile needs editing.
...
Don't escalate things that are actually fine. If the term is within the fallbacks in `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`, it doesn't need to go up.
...
Be specific. Not "escalate to legal leadership" — name the person or role from `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`. If the matrix doesn't name anyone for this situation, say so: "The escalation matrix doesn't cover [situation]. Suggest asking [GC name] who owns this."
...
```markdown
**Escalating to:** [name]
**Via:** [Slack #channel / email / meeting — per `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`]
...
**What our playbook says:** [quote from `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`]
...
The calibration for what warrants escalation lives in `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`, not in this skill. Check the playbook's stated position, its fallbacks, and its "automatic escalation regardless of dollar value" list:
...
If a term comes up that the playbook doesn't address, don't guess the threshold — ask the reviewing attorney whether this class of issue should escalate, and offer to record the answer in `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` so future reviews are consistent.
