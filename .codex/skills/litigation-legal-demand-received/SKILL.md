...
description: >
  Codex CLI version of litigation-legal:demand-received. Triage an inbound demand letter — extract fields, cross-check the portfolio, assess merit, present response options with a recommendation, and hand off to matter-intake or demand-intake if escalation is warranted. Use when the user says "we got a demand letter", "triage this demand", or shares an incoming demand to evaluate.
argument-hint: "[path-to-incoming] [--slug=custom-slug]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:demand-received` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-demand-received`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-demand-received

1. Read the incoming document from provided path.
2. Load `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` for portfolio cross-check.
3. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → risk calibration, landscape, demand-letter practice.
...
6. Write `~/.codex/claude-for-legal/litigation-legal/inbound/[slug]/triage.md`. Copy or link incoming to `~/.codex/claude-for-legal/litigation-legal/inbound/[slug]/incoming.[ext]`.
...
- `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` — scan for related matters (same counterparty, overlapping counterparties via entity relationships, or matter type + recent date)
- `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → risk calibration (for merit assessment), landscape (is the sender a frequent adversary?), demand-letter practice (house tone and response defaults)
...
- **Leverage and pressure** — are they credibly prepared to sue? Do they have capacity? Are they a repeat-litigant adversary per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`?
...
- Standalone → leave in `~/.codex/claude-for-legal/litigation-legal/inbound/`; no portfolio change.
