...
description: >
  Codex CLI version of employment-legal:policy-drafting. Draft an employment policy with state supplements where law differs across the jurisdictional footprint. Use when the user says "draft a [topic] policy", "we need a policy on", "update our [topic] policy", or names a policy gap.
argument-hint: "[policy topic — e.g., 'remote work', 'parental leave', 'PTO']"
---

# Codex Adaptation Notes

This skill was converted from `employment-legal:policy-drafting` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `employment-legal-policy-drafting`, or by describing the legal task in natural language.
...
# Codex skill: employment-legal-policy-drafting

1. Load `~/.codex/claude-for-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, handbook location.
...
`~/.codex/claude-for-legal/employment-legal/CLAUDE.md` → jurisdictional footprint, handbook location and format.
