...
description: >
  Codex CLI version of legal-clinic:client-comms-log. Log a client communication — call, email, text, letter, in-person, voicemail. Append-only per-case record with dated entries, direction, medium, summary, action items. Works alongside /client-letter and /status client. Use when logging a call or client email, reviewing a communication log, or asking "what did we tell [client] last time".
argument-hint: "[case-id] [--add (default) | --read | --summary | --patterns]"
---

# Codex Adaptation Notes

This skill was converted from `legal-clinic:client-comms-log` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `legal-clinic-client-comms-log`, or by describing the legal task in natural language.
...
# Codex skill: legal-clinic-client-comms-log

1. Use the workflow below.
2. Require case-id (prompt if not provided).
3. Route by flag:
   - `--add` (default): capture direction, medium, student, summary, action items, follow-up due. Confirm with user. Append (prepend most-recent-first) to `~/.codex/claude-for-legal/legal-clinic/client-comms/[case-id]/log.md`.
...
- `~/.codex/claude-for-legal/legal-clinic/client-comms/[case-id]/log.md` (if exists) — append target
- `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → not heavily read; this skill is case-scoped
...
**Append** to `~/.codex/claude-for-legal/legal-clinic/client-comms/[case-id]/log.md`. If the log doesn't exist, create it with a header:
