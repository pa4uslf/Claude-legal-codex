...
description: >
  Codex CLI version of litigation-legal:subpoena-triage. Triage a subpoena served on the company — classify it, analyze scope/burden/privilege, cross-check the portfolio, and produce an objections framework, compliance plan, and deadline calendar. Use when the user says "we got a subpoena", "served with a subpoena", or shares a subpoena, CID, or third-party document request to evaluate.
argument-hint: "[path-to-subpoena] [--slug=custom-slug]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:subpoena-triage` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-subpoena-triage`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-subpoena-triage

1. Read the subpoena from provided path.
2. Classify (third-party-docs / third-party-depo / party / CID / grand-jury).
3. If grand jury → stop, escalate per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. Otherwise continue.
4. Load `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` for cross-check. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → landscape, privilege conventions, escalation norms.
...
7. Write `~/.codex/claude-for-legal/litigation-legal/inbound/[slug]/triage.md`. Copy or link subpoena to `~/.codex/claude-for-legal/litigation-legal/inbound/[slug]/incoming.[ext]`.
...
- `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` — for related matter lookup and legal hold status
- `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → landscape (regulators we deal with), house privilege conventions, escalation norms
...
- Privilege log will be required — flag the format per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`
...
Output: `~/.codex/claude-for-legal/litigation-legal/inbound/[slug]/triage.md`.
...
**Privilege log format required:** [per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`]
...
**Before responding to the subpoena (serving objections, producing documents, appearing for deposition, or filing a motion to quash — any substantive response to the issuing party or court):** Read `## Who's using this` in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
- If classified as **grand jury subpoena** → stop, flag for escalation per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`, do not proceed with standard triage.
