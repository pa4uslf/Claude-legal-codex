...
description: >
  Codex CLI version of litigation-legal:privilege-log-review. First-pass privilege log review — make the obvious privilege calls and flag the hard ones for attorney review without making close calls. Use when the user says "review the privilege log", "priv log", "check privilege on these docs", or has a log to QA before production.
argument-hint: "[log file, or document set]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:privilege-log-review` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-privilege-log-review`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-privilege-log-review

1. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → review protocol, priv log format.
...
`~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → privilege log format, review protocol.

**Conflicts gate — unbypassable.** Before reviewing a privilege log, check `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
**Before the privilege log is served on the opposing party (the consequential act — this includes serving the log AND designating documents withheld or produced under a protective-order designation such as Confidential / Highly Confidential / AEO):** Read `## Who's using this` in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
