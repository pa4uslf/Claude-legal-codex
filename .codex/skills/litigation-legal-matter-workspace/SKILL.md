...
description: >
  Codex CLI version of litigation-legal:matter-workspace. Manage matter workspaces for multi-client practices — create, list, switch, close, or detach the active matter. Use when the user wants to create a new matter workspace, switch the active matter, list matters, archive a matter, or work at practice-level only without an active matter.
argument-hint: "<new | list | switch | close | none> [slug]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:matter-workspace` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-matter-workspace`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-matter-workspace
...
- `litigation-legal-matter-workspace close <slug>` — archive a matter (move to `~/.codex/claude-for-legal/litigation-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.codex/claude-for-legal/litigation-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.codex/claude-for-legal/litigation-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.codex/claude-for-legal/litigation-legal/matters/<slug>/` to `~/.codex/claude-for-legal/litigation-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.codex/claude-for-legal/litigation-legal/
