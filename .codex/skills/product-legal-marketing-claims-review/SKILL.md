...
description: >
  Codex CLI version of product-legal:marketing-claims-review. Review marketing copy for claims that need substantiation, reframing, or cutting. Use when the user says "review this marketing copy", "check these claims", "can we say this", "is this puffery or a problem", or pastes marketing content (landing pages, emails, ads, taglines).
argument-hint: "[paste copy, or file path]"
---

# Codex Adaptation Notes

This skill was converted from `product-legal:marketing-claims-review` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `product-legal-marketing-claims-review`, or by describing the legal task in natural language.
...
# Codex skill: product-legal-marketing-claims-review

1. Load `~/.codex/claude-for-legal/product-legal/CLAUDE.md` → Marketing claims standards.
...
Read `~/.codex/claude-for-legal/product-legal/CLAUDE.md` → `## Marketing claims`:
...
Per `~/.codex/claude-for-legal/product-legal/CLAUDE.md` — if comparative claims are "never," flag all of them. If "allowed with substantiation," check for the substantiation.
...
Prepend the work-product header from `~/.codex/claude-for-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
> **Before emitting "Ready to ship: Yes" (i.e., approving a claim for external use / publication):** Read `## Who's using this` in `~/.codex/claude-for-legal/product-legal/CLAUDE.md`. If the Role is Non-lawyer:
>
