...
description: >
  Codex CLI version of ai-governance-legal:policy-starter. Draft a firm AI usage policy from published model policies, adapted to your practice profile — a research-and-synthesis tool whose output is a draft for attorney review and adoption, not a finished policy. Use when user says "draft an AI policy", "we need an AI policy", "build an AI usage policy", "our firm needs a GenAI policy", or similar requests to generate a first-cut internal AI policy.
argument-hint: "[optional — scope hint, e.g. 'firm-wide', 'legal team only', 'update existing']"
---

# Codex Adaptation Notes

This skill was converted from `ai-governance-legal:policy-starter` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ai-governance-legal-policy-starter`, or by describing the legal task in natural language.
...
# Codex skill: ai-governance-legal-policy-starter

1. Read `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`. If the practice profile is unpopulated, stop and direct to `ai-governance-legal-cold-start-interview`.
