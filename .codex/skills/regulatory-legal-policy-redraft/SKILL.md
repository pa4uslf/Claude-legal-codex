...
description: >
  Codex CLI version of regulatory-legal:policy-redraft. Produce a proposed marked-up policy redraft that closes a gap found by regulatory-legal-gaps or regulatory-legal-policy-diff. A first draft for internal review — not for direct application to approved policy documents. Use when the user says "redraft the policy", "draft the policy fix", "mark up the policy", or when gap-surfacer hands off a gap for drafting.
argument-hint: "[GAP-ID or gap description]"
---

# Codex Adaptation Notes

This skill was converted from `regulatory-legal:policy-redraft` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `regulatory-legal-policy-redraft`, or by describing the legal task in natural language.
...
# Codex skill: regulatory-legal-policy-redraft

1. Load `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md` → policy library index + practice profile.
...
One of:
- A `GAP-ID` from the gap tracker — load the entry from `~/.codex/claude-for-legal/regulatory-legal/gap-tracker.yaml` (or the matter-level equivalent).
...
This skill reads the policy library index and owners from `~/.codex/claude-for-legal/regulatory-legal/CLAUDE.md`. When a value it needs is empty or still `[PLACEHOLDER]`:
