...
description: >
  Codex CLI version of ai-governance-legal:aia-generation. Run an AI impact assessment — structured intake, risk analysis, regulatory classification per regime in scope, policy consistency diff, and recommendation with conditions. Uses the house-style structure learned from the seed impact assessment in `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`. Use when user says "impact assessment for", "assess this AI use case", "run an AIA", "generate an AIA", "we need to document this AI system", "AI risk assessment for X", or follows a conditional triage result.
argument-hint: "[describe the use case or system, or pass a triage result]"
---

# Codex Adaptation Notes

This skill was converted from `ai-governance-legal:aia-generation` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ai-governance-legal-aia-generation`, or by describing the legal task in natural language.
...
# Codex skill: ai-governance-legal-aia-generation

1. Read `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`. Confirm impact assessment house style is populated.
...
6. Policy diff against `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` AI policy commitments.
...
- Is the use case in the elevated or high governance tier per `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
For each regime in `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` → `## Regulatory footprint` that applies to this system — **plus any regime surfaced by the re-derivation above** — research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`.** If none was captured, use this default:
...
|---|---|---|
| [commitment from `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md` AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in `~/.codex/claude-for-legal/ai-governance-legal/CLAUDE.md`.
Common drift:
