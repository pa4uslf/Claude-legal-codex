...
description: >
  Codex CLI version of corporate-legal:diligence-issue-extraction. Read VDR documents and extract issues per house categories and materiality thresholds, producing findings in house memo format. Use when user says "review the data room", "extract issues from [folder]", "diligence review", "what's in the VDR", or points at VDR documents.
argument-hint: "[VDR folder path or category name]"
---

# Codex Adaptation Notes

This skill was converted from `corporate-legal:diligence-issue-extraction` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `corporate-legal-diligence-issue-extraction`, or by describing the legal task in natural language.
...
# Codex skill: corporate-legal-diligence-issue-extraction

1. Load `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` + `~/.codex/claude-for-legal/corporate-legal/deals/[code]/deal-context.md`.
...
The VDR has 2,000 documents. Somewhere in there are the 30 that matter for the deal. This skill reads documents against the diligence categories and materiality thresholds from `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`, extracts issues, and writes them in house memo format.

## Load context

- `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` → Diligence structure (categories, materiality thresholds)
- `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` → Issues memo format (how findings are stated)
- `~/.codex/claude-for-legal/corporate-legal/deals/[code]/deal-context.md` → deal-specific thresholds, VDR location
...
Per `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` / deal-context thresholds. Don't review everything if the threshold says contracts >$X.
...
Per the finding template in `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`. If the seed memo used this:
...
- **To ai-tool-handoff:** If Luminance/Kira is in use per `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`, hand bulk contract review there. This skill handles the nuanced documents (side letters, amendments, anything the AI tool struggles with).
...
- It doesn't replace bulk AI review. For high-volume clause extraction, hand off to Luminance/Kira per `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`. This skill is for the judgment layer.
