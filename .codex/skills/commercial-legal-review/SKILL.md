...
description: >
  Codex CLI version of commercial-legal:review. Review a vendor agreement, NDA, or SaaS subscription against your playbook. Identifies the agreement structure from titles, routes to the right review skill (vendor-agreement-review, nda-review, saas-msa-review), and integrates the output into a single memo. Use when the user says "review this contract", "check this MSA", "is this NDA okay", "look at this SaaS agreement", or attaches an inbound agreement for review.
argument-hint: "[file path | Drive link | [CLM ID] | paste text]"
---

# Codex Adaptation Notes

This skill was converted from `commercial-legal:review` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `commercial-legal-review`, or by describing the legal task in natural language.
...
# Codex skill: commercial-legal-review

Reviews an inbound agreement against the playbook in `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`. Identifies the agreement structure from titles, selects the appropriate skill(s), and — if confirm_routing is enabled — checks with the user before proceeding.

## Instructions

1. **Load `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md`.** If placeholders present, stop and prompt: "Run `commercial-legal-cold-start-interview` first — I need to learn your playbook before I can review against it."

   Also read `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` → `## Review preferences` → `confirm_routing`. If the field is missing, treat it as `true`.
...
5. **Confirm routing if enabled.**
   ...
   If `confirm_routing` is `true` in `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` (or field is absent):
...
7. **Check for escalations:** If any issue exceeds the reviewer's authority per the `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` matrix, invoke **escalation-flagger** to route and draft the ask.
...
Add to `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` → `## Review preferences`:
...
Full review memo per the skill's format. Routing decision logged at the top. Deviation-by-deviation, specific redline language, named approver. Saved where `~/.codex/claude-for-legal/commercial-legal/CLAUDE.md` → House style says work product goes.
