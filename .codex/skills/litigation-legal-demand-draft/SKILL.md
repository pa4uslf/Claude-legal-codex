...
description: >
  Codex CLI version of litigation-legal:demand-draft. Draft a demand letter from a completed intake, gated on a privilege / FRE 408 / waiver / admission checklist, with a .docx output, post-send checklist, and an offer to create a matter. Use when the user says "draft the demand", "write the [type] letter", or has a finished demand intake ready to turn into a sendable draft.
argument-hint: "[slug] [--skip-gate] [--version=N]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:demand-draft` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-demand-draft`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-demand-draft

1. Load `~/.codex/claude-for-legal/litigation-legal/demand-letters/[slug]/intake.md`. Refuse if missing or strategic block empty (for material demands).
2. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → demand-letter practice, house style, seed-doc table.
...
5. Template select: seed doc if provided in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`; else soft template for the demand type.
6. Draft in-chat for review. Iterate until user approves.
7. Write `~/.codex/claude-for-legal/litigation-legal/demand-letters/[slug]/draft-v[N].docx` using the docx skill.
8. Write `~/.codex/claude-for-legal/litigation-legal/demand-letters/[slug]/checklist.md` (post-send checklist).
...
- `~/.codex/claude-for-legal/litigation-legal/demand-letters/[slug]/intake.md` — required; refuse to proceed if missing
...
- `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` — to check for existing related matters (same counterparty) and offer cross-link
...
Check `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → Demand-letter practice → seed-doc table for the intake's demand type.
...
7. **Privilege markings per house style.** Apply `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` privilege conventions exactly.

## Output

### Primary: `~/.codex/claude-for-legal/litigation-legal/demand-letters/[slug]/draft-v[N].docx`
...
### `~/.codex/claude-for-legal/litigation-legal/demand-letters/[slug]/checklist.md` — the post-send checklist
...
**Before the letter is sent (the consequential act):** Read `## Who's using this` in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
- **Default yes if ANY of:**
  ...
  - Desired outcome $$ ≥ `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` medium-severity band
...
> Create a tracked matter in `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml`? (default: [yes/no])
...
If user declines: update intake `status: drafted` (later `sent` when user confirms). The record stays in `~/.codex/claude-for-legal/litigation-legal/demand-letters/` only.
