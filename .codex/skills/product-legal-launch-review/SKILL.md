...
# Codex skill: product-legal-launch-review

1. Load `~/.codex/claude-for-legal/product-legal/CLAUDE.md` → framework + calibration. Stop if placeholders.
...
Read the PRD, check every category in this team's framework, calibrate against what actually blocks here (per `~/.codex/claude-for-legal/product-legal/CLAUDE.md`), and output a review in house format. Goal: a PM reads it and knows exactly what has to happen before they ship.
...
For each finding, check against the calibration table in ~/.codex/claude-for-legal/product-legal/CLAUDE.md:
...
Format per `~/.codex/claude-for-legal/product-legal/CLAUDE.md` → Launch review process → output format. Prepend the work-product header from `~/.codex/claude-for-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`). If no house format is specified:
...
> **Before emitting a "Clear to ship" or "Ship with conditions" call:** Read `## Who's using this` in `~/.codex/claude-for-legal/product-legal/CLAUDE.md`. If the Role is Non-lawyer:
>
...
- It doesn't retroactively calibrate. If this launch turns out fine (or badly) in a way that should update the calibration table, a human updates ~/.codex/claude-for-legal/product-legal/CLAUDE.md.
