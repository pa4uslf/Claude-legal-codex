...
description: >
  Codex CLI version of corporate-legal:material-contract-schedule. Build the material contracts disclosure schedule from diligence findings, applying the purchase agreement's Material Contract definition and formatting per the agreement's schedule format. Use when user says "build the contracts schedule", "disclosure schedule", "schedule 3.X", "material contracts list", or when drafting disclosure schedules.
argument-hint: "[purchase agreement path, or paste the Material Contract definition]"
---

# Codex Adaptation Notes

This skill was converted from `corporate-legal:material-contract-schedule` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `corporate-legal-material-contract-schedule`, or by describing the legal task in natural language.
...
# Codex skill: corporate-legal-material-contract-schedule
...
- `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` → materiality thresholds (may differ from the agreement definition — use the agreement's)
