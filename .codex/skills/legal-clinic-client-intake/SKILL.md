...
description: >
  Codex CLI version of legal-clinic:client-intake. Structured intake — practice-area templates, cross-area issue spotting, conflict flags, and triage classification. Produces a formatted case summary the student analyzes and the professor reviews. Does NOT decide case acceptance. Use when starting a new client intake, running an intake interview, or writing up a new client's situation.
argument-hint: "[optional: practice area hint]"
---

# Codex Adaptation Notes

This skill was converted from `legal-clinic:client-intake` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `legal-clinic-client-intake`, or by describing the legal task in natural language.
...
# Codex skill: legal-clinic-client-intake

1. Load `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → practice areas, intake templates, supervision style, flag triggers.
...
Each practice area asks different questions. Use the template from `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` for this area. Defaults if none provided:
...
Per whatever conflict-check process `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` describes. At minimum:
...
Per `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` supervision style and flag triggers. If formal queue or configurable flags are enabled, and a trigger is present (deadline mentioned, DV indicator, immigration status at issue, etc.), note the flag.
