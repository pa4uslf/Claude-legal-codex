...
description: >
  Codex CLI version of litigation-legal:matter-update. Append a dated event to a matter's history file and refresh the log row — captures new developments, status changes, risk re-assessments, deadline shifts, and settlement authority changes. Use when the user wants to log an update on a matter, note a development, or record a status change against the portfolio.
argument-hint: "[slug] [brief event description]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:matter-update` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-matter-update`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-matter-update

1. Follow the workflow and reference below.
2. Confirm slug exists in `~/.codex/claude-for-legal/litigation-legal/matters/` and `_log.yaml`.
...
4. Append dated entry to `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/history.md`.
...
- `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` — find the row
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/history.md` — append target
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/matter.md` — reference (don't rewrite)
- `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` — risk calibration (if re-assessing risk)
...
If the Strategy update is a **settlement acceptance** (the company is accepting a settlement offer, executing a settlement agreement, or authorizing acceptance in principle — not merely logging an offer made or received): Read `## Who's using this` in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
If materiality moves to `reserved` or `disclosed`, and the matter did not previously carry a reserve or disclosure, flag the event as requiring finance / audit-committee notification per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` materiality thresholds.
...
### Append to `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/history.md`
...
### Update `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml`
