...
description: >
  Codex CLI version of litigation-legal:oc-status. Generate weekly status-request email drafts to outside counsel across the active portfolio — markdown per matter, plus Gmail drafts when the MCP is available. Use when the user asks for OC status requests, weekly outside counsel check-ins, or wants per-matter status emails drafted from the portfolio log.
argument-hint: "[--all | --slug=foo | --no-gmail]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:oc-status` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-oc-status`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-oc-status
...
1. Load `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml`, filter per default rules (or per flags).
2. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → outside counsel directive style, signer defaults, budget posture.
3. Follow the workflow and reference below.
4. For each matter in scope: read `matter.md` + `history.md`, draft per-matter email.
5. Write markdown to `~/.codex/claude-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`.
6. If Gmail MCP authenticated: create Gmail drafts. Else: markdown-only, note in summary.
7. Write `~/.codex/claude-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md` — what ran, what was skipped and why.
...
- `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` — the filtering and field source
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/matter.md` — matter context (current posture, open questions)
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/history.md` — recent events to inform what to ask about
- `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → outside counsel directive style, signer name/email, budget posture
...
**Subject:** per house convention (from `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` outside counsel directive style; fallback: `[Matter: [matter name]] — Weekly status update`)
...
4. **Budget** — [monthly / quarterly / on-request per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` budget posture]. Where are we against [budget authorization from matter.md]? Any variance to flag?
...
[Signoff — name, role, contact. From `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` signer default for OC directives.]
```

Adapt tone per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` outside counsel directive style — some shops are "dear counsel" formal; others are first-name-and-bullets. Match.

## Output

### Markdown drafts

Write to: `~/.codex/claude-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`
...
**From:** [signer name / email from `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`]
**Subject:** [subject line]

> The work-product header above applies to this internal record. The outgoing email body below goes to outside counsel on a retained matter, which is itself a privileged communication — apply the house privilege marking (`~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` privilege conventions) at the top of the email sent, typically `Privileged & Confidential — Attorney-Client Communication / Attorney Work Product`, not this internal work-product header.
...
After processing all matters, write `~/.codex/claude-for-legal/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md`:
...
- **Enforce a minimum template.** If the house tone is "one line, first name, done," the draft honors that and skips the bulleted structure. Match `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`.
