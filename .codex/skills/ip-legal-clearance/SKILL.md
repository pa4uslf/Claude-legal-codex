...
description: >
  Codex CLI version of ip-legal:clearance. Trademark clearance first pass — knockout + similar-marks check producing a flag list, not a clearance opinion. Use when a new mark is proposed, when asked whether a mark is available or to run a knockout search, or when assessing likelihood-of-confusion factors before a full professional search. This skill never concludes a mark is clear.
argument-hint: "[describe the proposed mark, goods/services, and jurisdictions — or just the mark and I'll ask]"
---

# Codex Adaptation Notes

This skill was converted from `ip-legal:clearance` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ip-legal-clearance`, or by describing the legal task in natural language.
...
# Codex skill: ip-legal-clearance
...
1. Read `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`. If it
...
Before running clearance, read `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`. Pull:
...
If `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
Read `## Available integrations` from `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`:
...
Per the decision posture in `~/.codex/claude-for-legal/ip-legal/CLAUDE.md`:
...
Prepend the work-product header from `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` `## Outputs`.
...
- [routing per `~/.codex/claude-for-legal/ip-legal/CLAUDE.md` —
...
`~/.codex/claude-for-legal/ip-legal/matters/<matter-slug>/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`.
Otherwise write to
`~/.codex/claude-for-legal/ip-legal/outputs/clearance-<mark-slug>-YYYY-MM-DD.md`
