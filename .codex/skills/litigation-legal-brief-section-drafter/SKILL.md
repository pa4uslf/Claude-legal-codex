...
description: >
  Codex CLI version of litigation-legal:brief-section-drafter. Draft a brief section in house style, consistent with the case theory — every fact cited, every case checked, every argument tied to the theory. Use when the user says "draft the [section]", "write the statement of facts", "argument section on [issue]", or needs a first draft of a brief section.
argument-hint: "[section \\u2014 e.g., 'statement of facts', 'argument II']"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:brief-section-drafter` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-brief-section-drafter`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-brief-section-drafter

1. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → case theory, house style.
...
`~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → case theory, house style (citation format, structure, tone, length norms).

**Conflicts gate — unbypassable.** Before drafting, check `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` for the matter slug this skill is being invoked on. If the matter is not in `_log.yaml`, refuse and route:
...
Per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`:
...
**Before the brief is filed (the consequential act — this skill drafts, but the gate runs at the filing step regardless of who triggers it):** Read `## Who's using this` in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
