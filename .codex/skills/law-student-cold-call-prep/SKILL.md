...
description: >
  Codex CLI version of law-student:cold-call-prep. Prep for a cold-call — predict the professor's likely questions and drill them Socratically, flagging where you're shaky so you know what to re-read before class. Use when the user says "prep for class tomorrow", "cold call [case]", "what might [professor] ask on", or points at assigned reading.
argument-hint: "[case name, or paste case text, or path to reading]"
---

# Codex Adaptation Notes

This skill was converted from `law-student:cold-call-prep` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `law-student-cold-call-prep`, or by describing the legal task in natural language.
...
# Codex skill: law-student-cold-call-prep

1. Load `~/.codex/claude-for-legal/law-student/CLAUDE.md` → class list, professors, learning style.
...
- `~/.codex/claude-for-legal/law-student/CLAUDE.md` → current classes, professors, learning style
...
- Professor (from ~/.codex/claude-for-legal/law-student/CLAUDE.md class list — tone and focus vary by professor)
- Class / subject area
...
**Professor-specific flavor (from ~/.codex/claude-for-legal/law-student/CLAUDE.md notes):**
...
- **Predict jurisdiction-specific niche questions.** If the professor has known hobby horses, capture them in ~/.codex/claude-for-legal/law-student/CLAUDE.md class notes and the skill can weight accordingly; otherwise, it works from general patterns.
