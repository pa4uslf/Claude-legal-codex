...
description: >
  Codex CLI version of ip-legal:matter-workspace. Manage matter workspaces — create, list, switch, close, or detach the active matter. Use in multi-client private practice to keep one client's context separate from another, or when a substantive skill needs to know which matter it's working in.
argument-hint: "<new | list | switch | close | none> [slug]"
---

# Codex Adaptation Notes

This skill was converted from `ip-legal:matter-workspace` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `ip-legal-matter-workspace`, or by describing the legal task in natural language.
...
# Codex skill: ip-legal-matter-workspace
...
- `ip-legal-matter-workspace close <slug>` — archive a matter (move to `~/.codex/claude-for-legal/ip-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.codex/claude-for-legal/ip-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.codex/claude-for-legal/ip-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.codex/claude-for-legal/ip-legal/matters/<slug>/` to `~/.codex/claude-for-legal/ip-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.codex/claude-for-legal/ip-legal/
