...
description: >
  Codex CLI version of product-legal:cold-start-interview. Cold-start interview — connects to your launch tracker, reads past reviews, learns your risk calibration. Use on fresh install, when onboarding product counsel, or when the plugin config has placeholders. Run with --redo to re-interview, or --check-integrations to re-probe connectors only.
argument-hint: "[--redo] [--check-integrations to re-probe integrations only]"
---

# Codex Adaptation Notes

This skill was converted from `product-legal:cold-start-interview` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `product-legal-cold-start-interview`, or by describing the legal task in natural language.
...
# Codex skill: product-legal-cold-start-interview

1. Check `~/.codex/claude-for-legal/product-legal/CLAUDE.md` state.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.codex/claude-for-legal/cache/product-legal/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.codex/claude-for-legal/product-legal/CLAUDE.md` (create parent directories as needed). Show calibration table for confirmation.

## `--check-integrations`

Re-runs the integration availability check (launch tracker, document storage, Slack) and updates `## Available integrations` in `~/.codex/claude-for-legal/product-legal/CLAUDE.md`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
Read `~/.codex/claude-for-legal/product-legal/CLAUDE.md`:
- **Does not exist** → start the interview.
- **Contains `<!-- SETUP PAUSED AT: -->`** → greet the user and offer to resume from that section.
- **Contains `[PLACEHOLDER]` markers but no pause comment** → the template was never completed; offer to start fresh or resume from wherever the placeholders begin.
- **Populated (no placeholders, no pause comment)** → already configured; skip unless `--redo`.

The template structure lives at `product-legal/CLAUDE.md` — use it as the section scaffold. Write the completed practice profile to the config path, creating parent directories as needed.

If a CLAUDE.md exists at the old cache path `~/.codex/claude-for-legal/cache/product-legal/*/CLAUDE.md` but not at the config path, copy it forward.
...
For connectors that show as not connected, tell the user how to connect. Example phrasing: "Jira isn't connected. In Codex CLI: Settings → Connectors → Add → Jira → sign in. In Codex CLI: add the Jira MCP to your config or via `/mcp`. This plugin works without it — you'll paste PRDs and review docs directly — but connecting it lets the launch-watcher agent pull tickets automatically."
...
2. **Research connector prompt.** Say:
   ...
   > "Before your first launch review: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex CLI: authorize when a skill prompts you."
...
5. **Close with the changeability note.** Say:
   ...
   > "Done. Your configuration is at `~/.codex/claude-for-legal/product-legal/CLAUDE.md` — a plain-text file you can read and edit directly. Anything you answered can be changed:
   >
