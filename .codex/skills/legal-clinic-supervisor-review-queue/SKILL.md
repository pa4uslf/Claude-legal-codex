...
description: >
  Codex CLI version of legal-clinic:supervisor-review-queue. Professor's review queue — student output waits here for professor approval before going to clients or courts. Only active if "formal review queue" supervision style was chosen at setup; otherwise dormant. Use when the professor wants to see what's waiting for review, approve, edit-then-approve, or return an item.
argument-hint: "[--approve ID | --return ID 'note' | --edit ID]"
---

# Codex Adaptation Notes

This skill was converted from `legal-clinic:supervisor-review-queue` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `legal-clinic-supervisor-review-queue`, or by describing the legal task in natural language.
...
# Codex skill: legal-clinic-supervisor-review-queue

1. Check `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → supervision style. If NOT "formal review queue": explain the clinic is set up for [flags/lighter-touch], no formal queue exists, and how to switch.
...
**This skill is only active if `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` → Supervision style is "formal review queue."** Otherwise it's dormant — the cold-start interview asks the professor which model they want, and this is one of three options.
