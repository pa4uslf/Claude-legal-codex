...
description: >
  Codex CLI version of privacy-legal:matter-workspace. Manage matter workspaces — create, list, switch, close, or detach (practice-level). Keeps one client or engagement's context separate from every other for multi-client practitioners. Use when the user wants to open a new matter, switch matters, list matters, close/archive a matter, or work at practice-level only.
argument-hint: "<new | list | switch | close | none> [slug]"
---

# Codex Adaptation Notes

This skill was converted from `privacy-legal:matter-workspace` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `privacy-legal-matter-workspace`, or by describing the legal task in natural language.
...
# Codex skill: privacy-legal-matter-workspace
...
- `privacy-legal-matter-workspace close <slug>` — archive a matter (move to `~/.codex/claude-for-legal/privacy-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.codex/claude-for-legal/privacy-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.codex/claude-for-legal/privacy-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.codex/claude-for-legal/privacy-legal/matters/<slug>/` to `~/.codex/claude-for-legal/privacy-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.codex/claude-for-legal/privacy-legal/
