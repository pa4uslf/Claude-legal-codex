...
description: >
  Codex CLI version of employment-legal:matter-workspace. Manage matter workspaces — new, list, switch, close, or detach (practice- level). Creates, lists, switches, closes, and detaches the active matter so context from one client engagement never leaks into another. Use when a multi-client practitioner says "new matter", "switch matter", "list my matters", "close this matter", or needs to manage which matter is active.
argument-hint: "<new | list | switch | close | none> [slug]"
---

# Codex Adaptation Notes

This skill was converted from `employment-legal:matter-workspace` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `employment-legal-matter-workspace`, or by describing the legal task in natural language.
...
# Codex skill: employment-legal-matter-workspace
...
- `employment-legal-matter-workspace close <slug>` — archive a matter (move to `~/.codex/claude-for-legal/employment-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.codex/claude-for-legal/employment-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.codex/claude-for-legal/employment-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.codex/claude-for-legal/employment-legal/matters/<slug>/` to `~/.codex/claude-for-legal/employment-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.codex/claude-for-legal/employment-legal/
