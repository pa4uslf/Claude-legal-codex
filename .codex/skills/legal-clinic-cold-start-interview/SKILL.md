...
# Codex skill: legal-clinic-cold-start-interview

1. Check `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md`. If populated and no `--redo`, confirm before overwriting.
...
5. Migration: if a populated CLAUDE.md (no `[PLACEHOLDER]` markers) exists at `~/.codex/claude-for-legal/cache/legal-clinic/*/CLAUDE.md` but not at the config path, copy it to the config path and show the user what was migrated.
6. Write `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` including `## Who's using this` and `## Available integrations`. Show supervision choice and practice-area templates for confirmation.
...
Read `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md`:
...
Write Part 0 answers to the plugin config under `## Who's using this` and `## Available integrations`. If a populated CLAUDE.md exists at the old cache path `~/.codex/claude-for-legal/cache/legal-clinic/*/CLAUDE.md` but not here, copy it forward first.
...
6. **Before your first case review, connect a research tool.** Say: "Before your first case review or memo: connect a research tool. Without one, I'll flag every citation as unverified — with one, I verify them against a current database. In Cowork: Settings → Connectors. In Codex CLI: authorize when a skill prompts you."
...
> Done. Your clinic's configuration is at `~/.codex/claude-for-legal/legal-clinic/CLAUDE.md` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
