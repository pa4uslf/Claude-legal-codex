...
# Codex skill: privacy-legal-pia-generation

1. Load `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → PIA house style (trigger, structure, depth, sign-off).
...
Before writing a new PIA, check the outputs folder for prior work on the same feature, processing activity, or counterparty. Read `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → `## Outputs` for the path. Scan for:
...
**Carry severity from upstream as a floor** per the cross-skill severity floor rule in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → `## Shared guardrails`. A use-case-triage that rated the activity high-risk cannot become a PIA that concludes low-risk without stating why and what changed.
...
Read `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → `## PIA house style`. That has:
...
Check the trigger criteria in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md`. That is the team's house answer.
...
**Use the seed PIA structure from the config CLAUDE.md.** If none was captured, use this default. Prepend the work-product header from `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
Every PIA should cross-check against the privacy policy commitments in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md`. The common drift:
...
- **To the sign-off process:** Per `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → who approves PIAs.
...
**Before proceeding to submit a DPIA (or any equivalent impact assessment) to a regulator, supervisory authority, or enforcement body:** Read `## Who's using this` in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
