...
description: >
  Codex CLI version of privacy-legal:policy-monitor. Keep the privacy policy current with practice. Two modes: weekly sweep of saved PIAs, DPA reviews, and triage results to find policy drift; or direct query for a proposed new practice. Use when the user asks "does our policy cover this", "we want to start doing X — does the policy need updating", "run the policy monitor", "policy sweep", or wants to find where the privacy policy no longer matches what the team actually does.
argument-hint: "[describe a proposed new practice — or omit / use --sweep for crawl mode]"
---

# Codex Adaptation Notes

This skill was converted from `privacy-legal:policy-monitor` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `privacy-legal-policy-monitor`, or by describing the legal task in natural language.
...
# Codex skill: privacy-legal-policy-monitor

**Sweep mode** (no argument or `--sweep`):
1. Read `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → outputs folder path, policy document, last sweep date.
...
6. Update Last policy sweep date in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md`.

**Direct query mode** (with description argument):
1. Read `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → current policy commitments + actual policy document.
...
Update **Last policy sweep** in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` to today's date after completing the sweep.
...
`~/.codex/claude-for-legal/privacy-legal/CLAUDE.md`, so the next sweep only looks at new files.
