...
description: >
  Codex CLI version of legal-builder-hub:skills-qa. Evaluate a skill against the Legal Skill Design Framework — thirteen design parameters (including trust-surface, freshness, schema validation, and conflict detection), three legal failure modes, and a three-band verdict (Ready / Some Concern / Material Concerns). Use when deciding whether to trust a community skill before installing it, before deploying a first-party skill to your team, or whenever the user asks "should I trust this?" or "is this skill well-designed?". Runs automatically as part of legal-builder-hub-skill-installer.
argument-hint: "[skill path | SKILL.md path | paste content]"
---

# Codex Adaptation Notes

This skill was converted from `legal-builder-hub:skills-qa` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `legal-builder-hub-skills-qa`, or by describing the legal task in natural language.
...
# Codex skill: legal-builder-hub-skills-qa
...
- `~/.codex/claude-for-legal/legal-builder-hub/CLAUDE.md` → practice profile and installed skills list (provides context
...
- The skill's associated `CLAUDE.md` (template in the plugin directory, user config at `~/.codex/claude-for-legal/<plugin>/CLAUDE.md`) — if available, what practice profile the skill reads and depends on
...
3. **Config-override instructions** — text telling Claude to modify the user's
   ...
   configs, or `~/.codex/claude-for-legal/...` outside the skill's own
   directory.
4. **Out-of-scope reads** — instructions to read paths outside the skill's own
   directory and `~/.codex/claude-for-legal/<plugin>/`. Flag
