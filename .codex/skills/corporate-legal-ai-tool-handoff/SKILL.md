...
description: >
  Codex CLI version of corporate-legal:ai-tool-handoff. Detects when Luminance, Kira, or a similar bulk-review tool is in use, hands off the high-volume clause extraction to it, and QAs its output per the trust level in `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`. Use when user says "send to Luminance", "bulk review", "AI extraction", or when diligence-issue-extraction hits a high-volume category.
---

# Codex Adaptation Notes

This skill was converted from `corporate-legal:ai-tool-handoff` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `corporate-legal-ai-tool-handoff`, or by describing the legal task in natural language.
...
`~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` → AI-assisted review:
...
If `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` says this — it's rare.)
...
**Trust level:** [per `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`]
...
- It doesn't replace the tool's output with its own judgment entirely — if `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`, set at cold-start based on the team's experience with the tool.
