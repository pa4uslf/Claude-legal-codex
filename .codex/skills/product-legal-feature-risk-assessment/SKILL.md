...
description: >
  Codex CLI version of product-legal:feature-risk-assessment. Deeper risk assessment for a single feature or product area when the launch review found something that needs more than a line item. Structured analysis: what could go wrong, how likely, how bad, what mitigates it. Use when user says "deep dive on this risk", "risk assessment for [feature]", "what could go wrong with", or when launch-review flags a novel issue.
---

# Codex Adaptation Notes

This skill was converted from `product-legal:feature-risk-assessment` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `product-legal-feature-risk-assessment`, or by describing the legal task in natural language.
...
Before finalizing, check against `~/.codex/claude-for-legal/product-legal/CLAUDE.md` → Risk calibration:
