...
description: >
  Codex CLI version of litigation-legal:chronology. Build or update a chronology from declared document sources and uploads — dated events extracted, de-duped, and tagged by significance per the matter theory. Use when the user asks to build a chronology or timeline from a production or matter file, says "chron from the production" or "what happened when", or needs a working, statement-of-facts, or witness-specific timeline.
argument-hint: "[slug] [--format=working|sof|witness-[name]]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:chronology` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-chronology`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-chronology

1. Load `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/matter.md` → theory, pivot fact, key facts.
2. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → Document storage sources, default matter folder pattern.
3. Follow the workflow and reference below.
4. Identify sources in order: user-provided paths this session, default matter folder, declared sources from `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`.
...
8. Write `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/chronology.md` (or format variant per flag).
...
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/matter.md` → case theory, key facts, pivot fact (for significance tagging), key dates.
...
**Conflicts gate — unbypassable (`--matter` mode).** Before building the chronology, check `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` for the matter slug. If the matter is not in `_log.yaml`, refuse and route:
...
Location: `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/chronology.md`. Complete, tagged, annotated. The reference doc counsel works from.
