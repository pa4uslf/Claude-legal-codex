...
description: >
  Codex CLI version of product-legal:is-this-a-problem. Fast "is this a problem?" answer for the quick Slack question — pattern-matches against your calibration. Use when the user says "is this a problem", "quick question", "can we do X", "do I need legal review for", "sanity check", or pastes a PM's question that needs a same-minute fine / needs a look / hold call.
argument-hint: "[the question]"
---

# Codex Adaptation Notes

This skill was converted from `product-legal:is-this-a-problem` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `product-legal-is-this-a-problem`, or by describing the legal task in natural language.
...
# Codex skill: product-legal-is-this-a-problem

1. Load `~/.codex/claude-for-legal/product-legal/CLAUDE.md` → Risk calibration.
...
Read `~/.codex/claude-for-legal/product-legal/CLAUDE.md` → `## Risk calibration`. The whole point of this skill is pattern-matching against that table.
...
Slack triage replies are internal legal advice. If the reply is being pasted into a ticket, document, or channel that's broadly shared with non-legal, prepend the work-product header from `~/.codex/claude-for-legal/product-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`):
