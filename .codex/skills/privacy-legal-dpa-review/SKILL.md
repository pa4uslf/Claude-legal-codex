...
description: >
  Codex CLI version of privacy-legal:dpa-review. Review a Data Processing Agreement against your DPA playbook — auto-detects whether you're processor or controller and applies the right half of the playbook. Use when the user says "review this DPA", "check this data processing addendum", "customer sent their DPA", "is this DPA okay", or attaches a DPA.
argument-hint: "[file | Drive link | paste text]"
---

# Codex Adaptation Notes

This skill was converted from `privacy-legal:dpa-review` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `privacy-legal-dpa-review`, or by describing the legal task in natural language.
...
# Codex skill: privacy-legal-dpa-review

1. Load `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → DPA playbook. If placeholders, stop and prompt setup.
...
- **We are the processor** → customer is sending us their DPA → read `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → "When we are the processor" table
...
Before reviewing, check the outputs folder for prior work on this counterparty or processing activity. Read `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → `## Outputs` for the outputs folder path. Scan for:
...
**Carry severity from the upstream output as a floor** per the cross-skill severity floor rule in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → `## Shared guardrails`. A processing activity the triage rated 🔴 cannot be quietly downgraded to 🟢 in the DPA review; any demotion is stated and explained.
...
Read `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` → `## DPA playbook`. Also read `## Privacy policy commitments` — the DPA can't contradict what the privacy policy promises.
...
Prepend the work-product header from `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`).
...
**Before proceeding to sign or countersign a DPA (including returning an executed version, consenting to automatic execution on a counterparty platform, or instructing a signatory to execute):** Read `## Who's using this` in `~/.codex/claude-for-legal/privacy-legal/CLAUDE.md`. If the Role is Non-lawyer:
