...
description: >
  Codex CLI version of employment-legal:log-leave. Add a new leave to the leave register with the minimum information needed to start tracking deadlines. Use when an employee goes on leave and you want the tracker to watch designation, certification, and exhaustion clocks from day one.
argument-hint: "[describe the leave — employee/role, type, jurisdiction, start date]"
---

# Codex Adaptation Notes

This skill was converted from `employment-legal:log-leave` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `employment-legal-log-leave`, or by describing the legal task in natural language.
...
# Codex skill: employment-legal-log-leave

Adds a new leave entry to `~/.codex/claude-for-legal/employment-legal/leave-register.yaml` with the minimum
...
1. Read `~/.codex/claude-for-legal/employment-legal/CLAUDE.md` → jurisdiction table and Systems section.
...
3. Using the jurisdiction table in `~/.codex/claude-for-legal/employment-legal/CLAUDE.md`, look up the applicable leave
...
5. Write a new entry to `~/.codex/claude-for-legal/employment-legal/leave-register.yaml` using the leave register
