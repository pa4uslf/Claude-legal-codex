...
description: >
  Codex CLI version of product-legal:matter-workspace. Manage matter workspaces — new, list, switch, close, or detach (practice-level). Use when working across multiple clients or matters in private practice and you need to create, list, switch, close, or detach the active matter so context from one engagement doesn't leak into another.
argument-hint: "<new | list | switch | close | none> [slug]"
---

# Codex Adaptation Notes

This skill was converted from `product-legal:matter-workspace` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `product-legal-matter-workspace`, or by describing the legal task in natural language.
...
# Codex skill: product-legal-matter-workspace
...
- `product-legal-matter-workspace close <slug>` — archive a matter (move to `~/.codex/claude-for-legal/product-legal/matters/_archived/`, never delete)
...
3. Dispatch on the first token of `$ARGUMENTS`:
   - `new` → run the intake interview, write `~/.codex/claude-for-legal/product-legal/matters/<slug>/matter.md`, seed `history.md` and `notes.md`.
   - `list` → enumerate `~/.codex/claude-for-legal/product-legal/matters/*/matter.md`, print a table, mark the active matter.
   - `switch` → update the `Active matter:` line in the practice-level CLAUDE.md.
   - `close` → move `~/.codex/claude-for-legal/product-legal/matters/<slug>/` to `~/.codex/claude-for-legal/product-legal/matters/_archived/<slug>/`, log the close date in `history.md`.
...
```
~/.codex/claude-for-legal/product-legal/
