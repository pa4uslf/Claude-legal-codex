...
description: >
  Codex CLI version of legal-builder-hub:auto-updater. Check installed community skills for updates. Shows a diff and requires explicit approval before applying. Use when the user says "check for updates", "update my skills", "anything new for my installed skills", or when invoked from the registry-sync agent.
argument-hint: "[--apply to update all, otherwise notify only]"
---

# Codex Adaptation Notes

This skill was converted from `legal-builder-hub:auto-updater` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `legal-builder-hub-auto-updater`, or by describing the legal task in natural language.
...
# Codex skill: legal-builder-hub-auto-updater

1. Load `~/.codex/claude-for-legal/legal-builder-hub/CLAUDE.md` → installed skills + auto-update prefs.
...
`~/.codex/claude-for-legal/legal-builder-hub/CLAUDE.md` → installed skills (with version/commit SHA), update preferences (notify / manual).
...
Replace the installed skill files with the new version. Update `~/.codex/claude-for-legal/legal-builder-hub/CLAUDE.md` installed list with the new commit SHA. Backup the old version first (to `~/.claude/skills/.backups/[skill]-[old-sha]/`) in case of rollback.
