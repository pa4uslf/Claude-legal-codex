...
description: >
  Codex CLI version of litigation-legal:legal-hold. Issue, refresh, release, or report on legal holds — drafts the hold notice as .docx, updates legal_hold fields in _log.yaml, and calendars the next refresh. Use when the user says "issue a hold", "refresh hold", "release hold", or asks for a portfolio-wide hold status report.
argument-hint: "[slug] [--issue | --refresh | --release | --status]"
---

# Codex Adaptation Notes

This skill was converted from `litigation-legal:legal-hold` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `litigation-legal-legal-hold`, or by describing the legal task in natural language.
...
# Codex skill: litigation-legal-legal-hold

1. If `--status` (no slug): read `_log.yaml`, produce portfolio-wide hold report.
2. Otherwise: load `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/matter.md` + log row.
3. Load `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` → privilege markings, hold template pointer, escalation norms.
...
- `~/.codex/claude-for-legal/litigation-legal/matters/_log.yaml` — log row (legal_hold fields + status)
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/matter.md` — matter context (counterparty, facts, key custodians from internal_owners)
- `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` — house style for litigation hold template pointer, privilege marking, escalation norms
...
**Before issuing the hold to custodians (the consequential act):** Read `## Who's using this` in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Draft the notice** to each custodian, using the house template in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` if one is configured; otherwise the default template below.
...
DATE: [effective date]
TO: [custodian name]
FROM: [signer — per `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md` default]
...
**Writes:**
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/legal-hold-v1.docx` via the `docx` skill
- Appends to `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/history.md`:
  ```
...
**Writes:**
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/legal-hold-v[N].docx` (next version number)
- `history.md` entry
...
**Before releasing the hold (the consequential act — preservation obligations resume normal retention):** Read `## Who's using this` in `~/.codex/claude-for-legal/litigation-legal/CLAUDE.md`. If the Role is Non-lawyer:
...
**Writes:**
- `~/.codex/claude-for-legal/litigation-legal/matters/[slug]/legal-hold-release.docx`
- `history.md` entry
