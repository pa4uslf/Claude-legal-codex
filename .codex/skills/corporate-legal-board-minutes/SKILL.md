...
description: >
  Codex CLI version of corporate-legal:board-minutes. Drafts board or committee meeting minutes in your house format. Auto-detects upcoming board and committee meetings from your calendar, asks for the agenda and any slides or pre-read materials, and produces a complete draft in the format learned from your seed minutes. Also handles written consents in lieu of meetings. Trigger: "board minutes", "draft minutes", "upcoming board meeting", "committee minutes", "written consent", or calendar detection of an upcoming board or committee event.
---

# Codex Adaptation Notes

This skill was converted from `corporate-legal:board-minutes` in the Claude for Legal repository. In Codex CLI, invoke it by asking for the workflow by name, for example `corporate-legal-board-minutes`, or by describing the legal task in natural language.
...
- `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` → `## Board & Secretary` section:
...
- If `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` has no minutes format: run cold-start first. Do not proceed with a generic format.
...
**Directors present:**
- Pull from board composition in `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` as the starting point
...
Use the house format from `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`. Do not default to a generic format. The seed minutes are the template — replicate the structure, the header, the resolution language, the level of discussion detail.
...
**Previous minutes:**
Standard language: approval of minutes from prior meeting. Pull date of prior meeting from `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` board calendar if available, otherwise leave as [DATE OF PRIOR MEETING].
...
RESOLVED, that [resolution text in house language from `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`].
```
...
**Before adopting minutes as final:** Read `## Who's using this` in `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md`. If the Role is **Non-lawyer**:
...
Produce the full draft. The minutes themselves are a corporate record, not privileged; do not apply the work-product header to the minutes as circulated. The drafting notes, placeholder flags, and review checklist below are work product — prepend the work-product header from `~/.codex/claude-for-legal/corporate-legal/CLAUDE.md` `## Outputs` (it differs by user role — see `## Who's using this`):
